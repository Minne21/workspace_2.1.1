...
void FOC_Init(void)
{
    ...
    /* Speed loop */
        ...
        foc.pi_speed.ki = 5.0f;     // 1.1f -> 5.0f (정지 마찰력을 이겨내기 위해 속도 적분 강화)
        ...
        foc.pi_position.ki = 0.1f; // 💡 위치 제어 I항은 반드시 0.0f 로 설정!
...
//	foc.vq = 1.0f;
    ...
    if (speed_cnt >= SPEED_DIV)   // 20 * 50us = 1ms
    {
        ...
        if (foc.control_mode == 2)
        {
            ...
            // 💡 데드밴드: 오차가 0.01 rad (약 0.5도) 이내로 들어오면 속도 지시치 및 적분기 완전 리셋
            ...
            {
                // 💡 Anti-Windup: 오차가 너무 클 때는 적분 누적을 막고, 목표 부근(0.5 rad 이내)에서만 Ki 가동
                if (fabsf(pos_err) > 0.5f) {
                    foc.pi_position.integrator = 0.0f;
                }
