...
void FOC_Init(void)
{
    ...
    foc.speed_ref_rpm = 500.0f;   // 300 RPM target
    foc.speed_ref = foc.speed_ref_rpm * 0.10472f;  // RPM → rad/s

    foc.pi_speed.kp = 0.5f;
...
//	foc.vq = 1.0f;
    ...
    if (speed_cnt >= SPEED_DIV)   // 20 * 50us = 1ms
    {
        ...
        /* === [방어 필터 개선] === */
        float speed_drop = mech_rad_s - instant_rad_s;

        // 속도가 급격히 떨어지거나 튀는 이상치(Outlier)에만 이전 값 유지
        if (speed_drop > 50.0f || speed_drop < -50.0f)
        {
        	instant_rad_s = mech_rad_s;
        }
        ...
        // [우회전 전용] 속도 목표값도 음수가 되지 않도록 보호
        if (foc.speed_ref < 0.0f) {
            foc.speed_ref = 0.0f;
        }
