...
void FOC_Init(void)
{
    ...
    /* Speed loop */
        foc.speed_ref_rpm = 0.0f;
        foc.speed_ref = 0.0f;

        foc.pi_speed.kp = 0.5f;     // 1.5f -> 0.5f 로 다운 (부드러운 정지)
        foc.pi_speed.ki = 3.0f;     // 1.1f -> 5.0f (정지 마찰력을 이겨내기 위해 속도 적분 강화)
        foc.pi_speed.out_min = -3.0f;
        foc.pi_speed.out_max =  3.0f;
        foc.pi_speed.integrator = 0.0f;

        /* Position loop */
        foc.position_ref = 0.0f;
        foc.pi_position.kp = 10.0f;  // 3.0f -> 8.0f 로 상승 (복원력 증가)
        foc.pi_position.ki = 0.0f; // 💡 위치 제어 I항은 반드시 0.0f 로 설정!
        foc.pi_position.integrator = 0.0f;

        foc.pi_position.out_min = -104.7f;
        foc.pi_position.out_max =  104.7f;

        foc.control_mode = 2; 	// Position Mode
    }
...
float alpha_1ms = 0.03f;  // Filter for 1ms task (can be slightly higher than 50us version)
...
void FOC_PWM_ISR(void)
{
    ...
    if (!foc.aligned)
    {
        ...
        if (foc.align_timer >= ALIGN_TIME_S)
        {
            ...
            // 💡 Align 직후 현재 위치를 position_ref 및 last_total_count로 동기화
            last_total_count = motor_enc.total_count;
            foc.position_ref = (float)motor_enc.total_count * (2.0f * 3.14159265f / ENCODER_RES);
    ...
    // fmodf를 쓰면 값이 100이든 1000이든 0 ~ 6.28 범위로 한 번에 압축됩니다.
    control_elec_rad = fmodf(control_elec_rad, 6.2831853f);
    if (control_elec_rad < 0.0f) {
        control_elec_rad += 6.2831853f;
    }
...
//	foc.vq = 1.0f;
    ...
    if (speed_cnt >= SPEED_DIV)   // 20 * 50us = 1ms
    {
        ...
        // 카운터 차이를 라디안 변위로 변환
        float delta_theta = (float)delta_count * (2.0f * 3.14159265f / ENCODER_RES);
        ...
        /* =========================================================
         *                  1. POSITION CONTROL LOOP (1ms)
         * ========================================================= */
        float current_position_rad = (float)count_now * (2.0f * 3.14159265f / ENCODER_RES);

        if (foc.control_mode == 2)
        {
            float pos_err = foc.position_ref - current_position_rad;
            float abs_err = fabsf(pos_err);

            // 💡 1. 3.0 rad 같은 예민한 구간을 위한 데드밴드 설정 (약 0.3도 오차 범위 내 완전 정지)
            if (abs_err < 0.005f)
            {
                foc.speed_ref = 0.0f;
                foc.pi_position.integrator = 0.0f;
                foc.pi_speed.integrator = 0.0f; // 속도 적분기까지 리셋하여 완전 고정
            }
            else
            {
                // 💡 2. 목표 지점에 매우 가까워질 때(0.05 rad 이내) 위치 Kp를 둔화시켜 진동 방지
                float adaptive_kp = foc.pi_position.kp;
                if (abs_err < 0.05f) {
                    adaptive_kp *= 0.5f; // 목표 근처에서 P 게인 50% 다운
                }

                foc.speed_ref = pos_err * adaptive_kp; // P 제어
            }
        ...
        {
            // 속도 제어 모드일 때 기존 방식 유지
            foc.speed_ref = foc.speed_ref_rpm * 0.10472f;
        }

        /* =========================================================
                 *                  2. SPEED CONTROL LOOP (1ms)
                 * ========================================================= */

                /* ---------- SPEED PI ---------- */
                float iq_cmd = PI_Update(&foc.pi_speed, foc.speed_ref, mech_rad_s, 0.001f);

                /* ---------- Ramp (위치 제어 모드일 때는 Ramp 우회) ---------- */
                if (foc.control_mode == 2)
                {
                    // 💡 위치 제어 시에는 딜레이 없이 즉각 토크를 전달해야 미끄러지지 않음
                    iq_ref_ramp = iq_cmd;
                }
                else
                {
                    float ramp_step = 0.2f;
                    if (iq_ref_ramp < iq_cmd)      iq_ref_ramp += ramp_step;
                    else if (iq_ref_ramp > iq_cmd) iq_ref_ramp -= ramp_step;
                }
